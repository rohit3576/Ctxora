...
| Tests | `tests/test_*.py`, `tests/integration/` | unit (fakes) + fake-based e2e + live integration (skip-marked, `QUERYPULSE_IT=1`) |
...
- [~] Integration run vs live compose — **skipped by owner decision** ("no need for this"); instructions preserved in `demo/questions.md`; tests remain in `tests/integration/` behind `QUERYPULSE_IT=1`
